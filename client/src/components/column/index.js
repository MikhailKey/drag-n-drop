        ...
        {(provided, snapshot) => (
          ...
          >
            {props.tasks.map((task, index) => (
              <Task initialData={initialData} setInitialData={setInitialData} key={task.id} columnId={column.id} task={task} index={index} />
            ))}
            {provided.placeholder}
            {
              !openAdd ? (
                <Button onClick={() => setOpenAdd(true)} className={classes.addTaskButton}>
                  <AddIcon />
                </Button>
              ) : (
                  <ClickAwayListener onClickAway={() => setOpenAdd(false)}>
                    <form onSubmit={saveNewTask} className={classes.addTaskField}>
                      <TextField autoFocus={true} onChange={(e) => setTaskValue(e.target.value)} fullWidth id="standard-basic" label="Добавить задачу" variant="outlined" />
